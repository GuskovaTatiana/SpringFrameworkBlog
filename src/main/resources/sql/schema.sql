...
CREATE TABLE if not exists t_posts (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, -- идентификатор
    ...
    like_count INT DEFAULT 0, -- количество лайков к посту
...
CREATE TABLE if not exists t_comments_to_post(
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY, -- идентификатор
    post_id INT not null constraint fk_post_id references t_posts, -- идентификатор поста, к которому относится коммент
